...
func timer_func(w *Watcher, ticker *time.Ticker, quit chan struct{},redisClient *redis.Client) (){
  for {
    select {
    case <- ticker.C:
      fmt.Print("tick\n")
...
func setup_ticker(w *Watcher,duration int, redisClient *redis.Client) (chan struct{}, error){
